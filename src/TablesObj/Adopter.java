...
public class Adopter extends DBItem {
    ...
    public static ArrayList<ArrayList<Object>> getAll() {
        ...
        try (Connection con = DriverManager.getConnection(host, uName, uPass);
             CallableStatement st = con.prepareCall("{ CALL getAdopter() }")) {
            st.execute();
    ...
    public ResultSet getAllRS() {
        try {
            ...
            CallableStatement st = con.prepareCall("{ CALL getAdopterById(?) }");
            st.setInt(1, id);
            st.execute();
    ...
    public static void insert(int idUser, String firstName, String secondName,
        ...
        try (Connection con = DriverManager.getConnection(host, uName, uPass);
             CallableStatement st = con.prepareCall("{ CALL insertAdopter(?,?,?,?,?) }")) {
            st.setInt(1, idUser);
    ...
    public void update(String firstName, String secondName,
                       String firstSurname, String secondSurname) {
        try (Connection con = DriverManager.getConnection(host, uName, uPass);
             CallableStatement st = con.prepareCall("{ CALL updateAdopter(?,?,?,?,?) }")) {
