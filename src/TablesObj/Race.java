...
public class Race extends DBItem {
    ...
    public static ResultSet getByPetType(int idPetType) {
        try {
            ...
            CallableStatement stmt = con.prepareCall("{ CALL getRaceByPetType(?) }");
    ...
    public static ResultSet getAll() {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL getRace() }");
            return stmt.executeQuery();
        } catch (SQLException ex) { LOG.log(Level.SEVERE, null, ex); }
        return null;
    }
 
    public static void insert(String name, int idPetType) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL insertRace(?, ?) }");
    ...
    public void updateItem(int idRace, String name, int idPetType) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL updateRace(?, ?, ?) }");
    ...
    public void deleteItem(int selectedId) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL deleteRace(?) }");
