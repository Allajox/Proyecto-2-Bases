...
public class UserXBlackList extends DBItem {
    ...
    public static ResultSet getAll() {
        try {
            ...
            CallableStatement stmt = con.prepareCall("{ CALL getUserXBlackList() }");
    ...
    public static void insert(String reason, int idUser, int idReport) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL insertUserXBlackList(?, ?, ?) }");
    ...
    public static void delete(int idReport, int idUser) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL deleteUserFromBlackList(?, ?) }");
    ...
    public static void update(String reason, int idUser, int idReport) {
        try {
            Connection con = DriverManager.getConnection(host, uName, uPass);
            CallableStatement stmt = con.prepareCall("{ CALL updateUserXBlackList(?, ?, ?) }");
            stmt.setString(1, reason);
            stmt.setInt(2, idUser);
            stmt.setInt(3, idReport);
            stmt.execute();
